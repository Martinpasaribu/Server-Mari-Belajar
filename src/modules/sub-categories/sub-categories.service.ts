...
export class SubCategoriesService {
  ...
  async create(createDto: CreateSubCategoryDto): Promise<SubCategory> {
    ...
    const existing = await this.subCategoryModel.findOne({ slug, isDeleted: false }).exec();
  ...
  async findAll(query: any = {}): Promise<SubCategory[]> {
    return await this.subCategoryModel
      ...
      .populate('category_key', 'name') // Populate kategori
      .populate('enrolled_users')      // Pastikan field ini ada di SubCategorySchema
      .sort({ order: 1 })
      .exec();
  }
